...
const GrafoFilmes = () => {
  ...
  const encontrarCaminhoMaisCurto = () => {
    ...
    if (!grafo.listaAdjacencia.has(atorDe) || !grafo.listaAdjacencia.has(atorPara)) {
    ...
    if (resultado) {
      const caminhoFormatado = resultado.join(' -> ');
      const comprimentoDoCaminho = resultado.length - 1;
      console.log(`Caminho encontrado: ${caminhoFormatado}`);
      setCaminho(`${caminhoFormatado} (Comprimento: ${comprimentoDoCaminho})`);
    } else {
      setCaminho('Caminho não encontrado.');
    }
  };
  ...
  const encontrarTodosCaminhosMenoresQue6 = () => {
    ...
    setIsLoading(true);
    const resultados = grafo.procurarTodosOsCaminhos(atorDe, atorPara, 6);
  ...
  return (
    <div>
      <div class="menuGrafos">
        ...
        <button class="btn btn-dark" onClick={criarGrafo}>
          Criar Grafo
        </button>
        <input
          class="form-control"
        ...
        />
        <button class="btn btn-dark" onClick={encontrarCaminhoMaisCurto}>
          Encontrar Caminho Mais Curto
        </button>
        <button class="btn btn-dark" onClick={encontrarTodosCaminhosMenoresQue6}>
          Encontrar Todos Caminhos Até 6 Arestas
        </button>
      </div>
      <div>
